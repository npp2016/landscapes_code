...
df = data.frame(x=hmn$longitude, y = hmn$latitude)
...
hmnmap +
  geom_point(aes(x = longitude, y = latitude, colour = status, size = No_of_years),
             data = hmn)

hmnmap +
  geom_point(aes(x = longitude, y = latitude, colour = status),
             data = hmn) +
